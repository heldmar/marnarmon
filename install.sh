...
# Constants
# --------------------------------------------------------------------------- #
PREFIX="/opt/servermon"          # code + venv live here
...
if [ ! -d "${SRC_DIR}/servermon" ]; then
    err "Cannot find source at ${SRC_DIR}/servermon — run install.sh from the repo root."
    exit 1
fi
...
echo -e "${BOLD}ServerMon host installer${RESET}"
...
mkdir -p "${PREFIX}"
rm -rf "${PREFIX}/servermon"
cp -r "${SRC_DIR}/servermon" "${PREFIX}/servermon"
...
mkdir -p "${CONFIG_DIR}"
{
    echo "# ServerMon config — generated by install.sh on $(date -Is)"
    echo "host:"
    echo "  name: \"${HOST_NAME}\""
    echo "collection:"
...
render_unit "${SRC_DIR}/systemd/servermon-collector.service"
render_unit "${SRC_DIR}/systemd/servermon-collector.timer"
render_unit "${SRC_DIR}/systemd/servermon-api.service"

systemctl daemon-reload
systemctl enable --now servermon-collector.timer >/dev/null 2>&1
systemctl enable --now servermon-api.service >/dev/null 2>&1
...
if sudo -u "${SERVICE_USER}" SERVERMON_CONFIG="${CONFIG_FILE}" \
        "${PREFIX}/venv/bin/python" -m servermon.collect; then
    ok "Collector ran successfully"
else
    err "Collector failed — check: journalctl -u servermon-collector.service"
fi
...
if command -v curl >/dev/null 2>&1; then
    if curl -fsS "${auth_header[@]}" "http://${check_host}:${API_PORT}/health" >/dev/null 2>&1; then
        ...
        warn "API not responding yet — check: journalctl -u servermon-api.service"
    fi
else
...
# --------------------------------------------------------------------------- #
echo
echo -e "${GREEN}${BOLD}ServerMon installed.${RESET}"
echo "  API:        http://${check_host}:${API_PORT}"
echo "  Endpoints:  /health  /metrics/current  /metrics/history?window=24h"
echo "  Config:     ${CONFIG_FILE}"
echo "  Database:   ${DB_PATH}"
echo "  Collector:  every ${INTERVAL} min (systemctl status servermon-collector.timer)"
if [ -n "$API_TOKEN" ]; then
    echo
...
echo
echo "  Logs:   journalctl -u servermon-api.service -f"
echo "          journalctl -u servermon-collector.service -f"
